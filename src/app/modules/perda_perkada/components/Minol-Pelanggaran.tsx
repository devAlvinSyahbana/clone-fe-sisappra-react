...
export function Minol_Pelanggaran() {
    ...
    const [qParamFindPelanggaran, setUriFindPelanggaran] = useState({ strParamPelanggaran: '' })

    const dataPelanggaran = async (page: number) => {
        const response = await axios.get(
            `${MASTERDATA_URL}/jenis-perda-perkada${qParamFindPelanggaran.strParamPelanggaran}?%24top=200&%24select=id%2C%20jenis_pelanggaran`
        )
        const filter = await axios.get(
            `${MASTERDATA_URL}/map-master-perda/jenis-kegiatan?%24filter=jenis_kegiatan_id%20eq%20%2716%27&%24select=id%2C%20perda_id%2C%20jenis_kegiatan_id`
        )
        ...
        return [pelanggaran, setPelanggaran] as const
    }
