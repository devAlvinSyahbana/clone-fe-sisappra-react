...
export const DtPerdaPerkada: FC<any> = ({
      ...
      for (let i = 0; i < jenis.length; i++) {
        jumlahJenis = totalKegiatan.filter(
          (item: any) =>
            ...
            item.tindak_lanjut__administrasi__penyelesaian_id === jenis[i]
        ).length
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (record: any) => (
        <GetPerJenis
          row={record.jenis_penertiban}
          jenis={[6, 11, 12, 22, 27, 28, 30, 32, 35, 37, 40]}
        />
      ),
    },
    {
      ...
      selector: (row: any) => row.jenis_penertiban,
      cell: (record: any) => (
        <GetPerJenis row={record.jenis_penertiban} jenis={[2, 3, 10, 13, 14, 18, 19, 20, 21, 24]} />
      ),
    },
    {
    ...
    },
    {
      ...
      cell: (record: any) => (
        <GetPerJenis
          row={record.jenis_penertiban}
          jenis={[1, 4, 5, 7, 9, 17, 23, 25, 26, 29, 32, 33, 34, 36, 39, 41, 42, 43]}
        />
      ),
    },
    {
...
export const DtPerdaPerkadaPelaksana: FC<any> = ({
  ...
  const GetPerJenis = ({row, jenis}: any) => {
    ...
    if (row && jenis) {
      ...
      for (let i = 0; i < jenis.length; i++) {
        jumlahJenis = totalKegiatan.filter(
          (item: any) =>
            item.kegiatan__kota_id === row && item.kegiatan__jenis_kegiatan_id === jenis[i]
        ).length
        countJumlah += jumlahJenis
      }
    }
  ...
  const columns = [
    ...
    },
    {
      ...
      selector: (row: any) => row.no,
      cell: (record: any) => (
        <GetPerJenis row={record.no} jenis={[6, 11, 12, 22, 27, 28, 30, 32, 35, 37, 40]} />
      ),
    },
    {
      name: 'Penutupan/Penyegelan',
      ...
      selector: (row: any) => row.no,
      cell: (record: any) => (
        <GetPerJenis row={record.no} jenis={[2, 3, 10, 13, 14, 18, 19, 20, 21, 24]} />
      ),
    },
    {
      name: 'Pencabutan Izin',
