...
import {DtPenertibanBangunan} from './datatables/data-table-laporan-penertiban-bangunan'
...
interface penertibanBangunanInterface {
  no: number
  pelaksana: string
  rekomtek: number
  diterbitkan: number
  bongkar_sendiri: number
  terbit_ijin: number
  dikembalikan: number
  belum_dibongkar: number
}
...
export function LaporanPenertibanBangunan() {
  ...
  const [data, setData] = useState<penertibanBangunanInterface[]>([])
  ...
  const dataPerdaPerkada = (page: number) => {
    axios
      ...
      .then((res) => {
        ...
        //   item.serial = index + 1
        // })
        const arr: penertibanBangunanInterface[] = [
          {
            no: 1,
