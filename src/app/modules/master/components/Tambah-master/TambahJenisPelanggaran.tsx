...
export const JENIS_PELANGGARAN_URL = `${API_URL}/master/Jenis-pelanggaran` //http://localhost:3000/master/jenis-korban-jiwa

export function TambahPelanggaran() {
        ...
        if (response) {
          if (selectedFile) {
            ...
            const responseFile = await axios.post(
              `${JENIS_PELANGGARAN_URL}/upload-file/${response.data.data.return_id}`,
              formData
            )
            if (responseFile) {
              ...
              navigate('/master/JenisPelanggaran', {replace: true})
            }
            return
          }
          Swal.fire({
